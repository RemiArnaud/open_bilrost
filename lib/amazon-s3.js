...
module.exports = (amazon_client, cache, config) => {
    ...
    const download = (key, location_path) => promisify(fs.mkdirs)(path.dirname(location_path))
        ...
        .then(() => cache.read(key, location_path), () => simple_client({
            method: 'get',
            path: key,
            id: 'download'
        })
            .then(req => fs.ensureDir(cache.get_path())
                .then(() => new Promise((resolve, reject) => {
                    const write_stream = fs.createWriteStream(cache.get_path(key));
                    req.on('result', (err, res) => {
                        if (err) {
                            reject(err);
                        }
            ...
            .then(output => {
                if (output.hash === key) {
                    return promisify(fs.writeFile)(location_path, output.data);
                } else {
