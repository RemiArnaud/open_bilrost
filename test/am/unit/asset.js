...
const mock_rpc = require('../../util/mocks/rpc');
...
describe('Run set of test for asset management methods', function () {
    ...
    const test_level = {
        ...
        "comment": "",
        "tags": [],
        "main": "/resources/test/test_001.assembly",
        "dependencies": [
    ...
    const ifs_map = {
        "readJson" : {
            ...
            ".bilrost/assets/prefab/mall.prefab": () => Promise.resolve({
                ...
                comment: '',
                tags: [],
                main: '/resources/test/test01.assembly',
                dependencies: [],
                semantics: []
            }),
            ".bilrost/assets/prefab/test_1_1_0.prefab": () => Promise.resolve({
                ...
                tags: [ 'TEST' ],
                main: '/resources/test/test03.assembly',
                dependencies: [ '/resources/test/prefab/test.assembly' ],
                semantics: []
            })
        },
        "access" : {
            "test/test_001.assembly" : () => Promise.resolve(),
            "test/a/test_005.assembly" : () => Promise.resolve(),
            "test/b/test_005.assembly" : () => Promise.resolve(),
            "mall/mall_demo.assembly" : () => Promise.resolve(),
            "resources/test/a/test_005.assembly" : () => Promise.resolve(),
            "test/unexisted_folder/test_001.assembly" : () => Promise.reject("Can't be accessed!"),
            "b/test_005.assembly" : () => Promise.reject("Can't be accessed!")
        },
        "getDirectories" : {
    ...
    before("create fixtures", function(done) {
        ...
        test_util.create_eloise_fixtures()
            ...
            .then(() => {
                ...
                test_util.write_eloise_resource_file('/test/test_001.assembly', 'foo');
    ...
    describe('Searching with #find_asset_by_ref', () => {
        ...
        it('retrieves assets in root namespace with query to find dependency', function(){
            const query = 'dependency: /resources/test/test.assembly';
            return Asset.find_asset_by_ref(this_workspace, '/assets/', {q: query}).then(result => result.output)
                .then(asset_list => {
                    asset_list.totalItems.should.equal(1);
                    asset_list.items.length.should.equal(1);
    ...
    describe('Creating assets!', function () {
        ...
        it('Create an asset', function(done){
            this.timeout(3*this.timeout()); // = 3 * default = 3 * 2000 = 6000
            const asset = {
                main: "/resources/test/a/test_005.assembly",
                comment: "This a test asset",
                tags: ["test", "hello"],
                dependencies: ["/resources/test/test_001.assembly"],
                semantics: []
            };
            const asset_ref = '/assets/levels/test_002.level';

            asset_instance.create(asset_ref, asset, mock_rpc).then(function(asset){
                should.exist(asset);
        ...
        it('Cannot create an asset with invalid asset data paths', function(done){
            const asset_data = {
                main: "/resources/b/test_005.assembly",
                comment: "This a test asset",
                tags: ["test", "hello"],
                dependencies: ["/resources/test/unexisted_folder/test_001.assembly"],
                semantics: []
            };
            const asset_ref = '/assets/levels/test_002.level';
        ...
        it('Cannot create an asset with already referenced main dependency', function(done){
            ...
            asset_instance.create(asset_ref, asset_data, mock_rpc).then(function(){
                done("An asset shouldn't be created");
            }).catch(function(error){
                should.exist(error);
                should.equal(error.statusCode, 400);
    ...
    describe('Renaming assets!', function () {
        ...
        const test_002 = {
            ...
            "comment": "",
            "tags": [],
            "main": "/resources/prefab/test.assembly",
            "dependencies": [
...
*/
        ...
        it('Cannot rename an asset with an unexisted ref', function(done){
            ...
            asset_instance.rename("/assets/invalid_ref", new_asset_ref, test_level.meta.modified).then(function(asset){
        ...
        it('Cannot rename an asset with an unvalid ref', function(done){
            ...
            asset_instance.rename(test_003.meta.ref, new_asset_ref, test_level.meta.modified).then(function(asset){
                done("this asset shouldn't exist");
            }).catch(function(error) {
                should.exist(error);
                should.equal(error.statusCode, 400);
        ...
        it('Cannot rename an asset with a none matching modified date', function(done) {
            ...
            asset_instance.rename(test_003.meta.ref, new_asset_ref, "2016-03-18T10:54:05.860Z").then(function(asset){
                done("this asset shouldn't exist");
            }).catch(function(error) {
                should.exist(error);
        ...
        it('Cannot rename an asset with an invalid new ref', function(done) {
            ...
            asset_instance.rename(test_003.meta.ref, "/invalid_ref", test_level.meta.modified).then(function(asset){
                done("this asset shouldn't exist");
            }).catch(function(error){
                should.exist(error);
                should.equal(error.statusCode, 400);
