...
function Status_manager (workspace) {
    ...
    this.get_status = ref => {
        return this.get_statuses().find(status => status.get_ref() === ref);
    };

    this.update_and_retrieve_status = () => {
        return this.sync_all_status_from_database().then(() => this.get_statuses());
    };

    this.sync_all_status_from_database = () => {
        return Promise.all(this.get_statuses().map(status => status.sync_from_database()));
    };

    this.get_general_status = () => {
        return this.sync_all_status_from_database()
            .then(() => {
    ...
    this.check_overall_validation = () => {
        return this.asset_repo_manager.validate_vcs()
